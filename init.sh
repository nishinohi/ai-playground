...
# Cloudflare Workers + D1 + KV Local Development Initialization Script

set -e

echo "🚀 Initializing Cloudflare Workers local development environment..."
...
init_db() {
    ...
    # Wait for wrangler to be ready by checking if the port is open
    echo "⏳ Waiting for wrangler to be ready..."
    MAX_ATTEMPTS=30
    ATTEMPT=0
    while ! nc -z localhost 8787 2>/dev/null; do
        if [ $ATTEMPT -ge $MAX_ATTEMPTS ]; then
            echo "❌ Timeout waiting for wrangler to start"
            exit 1
        fi
        ATTEMPT=$((ATTEMPT + 1))
        sleep 1
    ...
    curl -s http://localhost:8787 > /dev/null 2>&1 || true
    ...
    echo "✅ Found SQLite database at: $SQLITE_PATH"

    # Create .env.local-build file
    echo "📝 Creating .env.local-build file..."
    cat > .env.local-build << EOF
D1_LOCAL_URL=$SQLITE_PATH
EOF

    echo "✅ Created .env.local-build with D1_LOCAL_URL"

    # Run database migrations
    echo "🗄️  Running database migrations..."
    npm run db:migrate-local-build

    # Create .dev.vars if it doesn't exist
    if [ ! -f ".dev.vars.local-build" ]; then
        echo "📝 Creating .dev.vars.local-build template..."
        cat > .dev.vars.local-build << EOF
# Google OAuth credentials
CLIENT_SECRET=''
CLIENT_ID=''
SESSION_SECRET='$(openssl rand -base64 32)'
EOF
        echo "⚠️  Please update .dev.vars.local-build with your Google OAuth credentials"
    fi

    echo ""
    echo "✅ Initialization complete!"
    echo ""
    echo "🚀 Wrangler is running on http://localhost:8787"
    echo ""
    echo "Next steps:"
    echo "1. Update .dev.vars.local-build with your Google OAuth credentials (if not already done)"
    echo "2. Visit http://localhost:8787 to access your application"
