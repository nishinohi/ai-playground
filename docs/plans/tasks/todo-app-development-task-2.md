...
- [x] `app/lib/todo/task-types.ts` - 型定義（新規作成）
- [x] `app/lib/todo/task-validation.ts` - バリデーション（新規作成）
- [x] `app/lib/todo/task-service.ts` - サービス層（新規作成）
...
- [x] 全型定義が正しくエクスポートされる
- [x] バリデーション関数が期待通りに動作する
- [x] サービス層ヘルパー関数が型安全に動作する
